...
var whiteRemoved = 0;
var blackRemoved = 0;
let lastPlayedCoordinates = null; // Coordenadas da ultima jogada realizada (3-4)
...
/*boardPieceCircles.forEach(boardPieceCircle => {
    boardPieceCircle.addEventListener('click', (event) => {
        ...
        if (positionedBlackPieces + positionedWhitePieces === TOTAL_NUMBER_OF_PIECES - 1){
            ...
            let array = coordinatesToArrayPosition(lastPlayedCoordinates);
        ...
        if (positionedBlackPieces + positionedWhitePieces === TOTAL_NUMBER_OF_PIECES){
            ...
            lastPlayedCoordinates = getCoordinatesFromCircle(boardPieceCircle);
            let array = coordinatesToArrayPosition(lastPlayedCoordinates);
            //if (currentPlayerColor == 'white')  notify(array[0],array[1],'white',currentWhitePieces);
            //else if (currentPlayerColor == 'black')  notify(array[0],array[1],'black',currentBlackPieces);
        ...
        if (boardPieceCircle.getAttribute('occupied') === 'true') {
            //lastPlayedCoordinates = getCoordinatesFromCircle(boardPieceCircle);
            //console.log("Last Played Coordinates:", lastPlayedCoordinates);
            //let array = coordinatesToArrayPosition(lastPlayedCoordinates);
            //console.log(array); // obter posicao no array da coordenada
            //notify(array[0],array[1]); // notify quando está na fase de drop
            //console.log(arrayPositionToCoordinates(array));
        }
    });
});
*/
...
const quadrado1 = ['1-1', '1-4', '1-7', '4-1', '4-7', '7-1', '7-4', '7-7'];
const quadrado2 = ['2-2', '2-4', '2-6', '4-2', '4-6', '6-2', '6-4', '6-6'];
const quadrado3 = ['3-3', '3-4', '3-5', '4-3', '4-5', '5-3', '5-4', '5-5'];

function coordinatesToArrayPosition(coordinates) {
    // Checar em qual quadrado a coordenada está e retornar a posição
    if (quadrado1.includes(coordinates)) {
        return [0, quadrado1.indexOf(coordinates)];
    } else if (quadrado2.includes(coordinates)) {
        return [1, quadrado2.indexOf(coordinates)];
    } else if (quadrado3.includes(coordinates)) {
        return [2, quadrado3.indexOf(coordinates)];
    } else {
        return null; // Coordenada inválida
    }
}

function arrayPositionToCoordinates(arrayPosition) {
    const [quadrado, posicao] = arrayPosition;

    // Retornar a coordenada com base no quadrado e na posição
    if (quadrado === 0) {
        return quadrado1[posicao];
    } else if (quadrado === 1) {
        return quadrado2[posicao];
    } else if (quadrado === 2) {
        return quadrado3[posicao];
    } else {
...
function addGameResultToTable() {
    ...
    // Find the classifications table
    const table = document.getElementById("classifications-table");

    // Create a new row
    const newRow = document.createElement("tr");

    // Create and append the Player column
    const playerCell = document.createElement("td");
    playerCell.textContent = results.player;
    newRow.appendChild(playerCell);

    // Create and append the Match Result column
    const matchResultCell = document.createElement("td");
    matchResultCell.textContent = results.matchresult;
    newRow.appendChild(matchResultCell);

    // Create and append the Board Size column
    const boardSizeCell = document.createElement("td");
    boardSizeCell.textContent = results.boardSize;
    newRow.appendChild(boardSizeCell);
