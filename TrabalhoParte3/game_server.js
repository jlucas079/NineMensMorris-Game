...
module.exports = class game_server{
    ...
    canDothis(square, position, nick) {
		...
		if (!this.remove){
			if (square == this.rselected && position==this.cselected){return 'valid';}
			if (this.board.board[square][position]!=0)return 'Invalid move: non empty cell';
			if (this.board.Repeat(this.rselected,this.cselected,square,position)){return 'Invalid move: cannot return immediastly to this cell';}
			if (this.board.CanMove(this.rselected, this.cselected, square, position)){return 'valid';}
    ...
    Dothis(square,position,nick){
		...
		if(!this.remove){
			if(square == this.rselected && position==this.cselected){this.Unselect(square,position);return;}
			this.board.Move(this.rselected,this.cselected,square,position);
